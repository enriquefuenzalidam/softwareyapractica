...
const softwaresLista = softwLista.map(softw => {
  ...
  return (
    <li data-aos-once="true" data-aos='fade' className={`relative flex flex-col rounded-r-3xl p-4 border-t-2 border-black border-opacity-5 bg-gradient-to-b from-[rgb(217,239,252,0.3)] to-transparent shadow-md shadow-[rgba(0,0,0,0.5)] `} key={softw.id}>
      ...
      {softw.softPrec && (
        <div className={` relative `}>
          ...
          {quantity !== 0 && (
            <p>
              <span className={` absolute top-full left-full -translate-x-[calc(100%)] -translate-y-[calc(100%+1rem)] text-center py-1 px-3 inline-block font-bold text-lg md:text-xl rounded-sm overflow-hidden bg-[#faae3b] bg-opacity-70 shadow-inner shadow-neutral-500 rounded-l-lg`}>
                {quantity}
              </span>
            </p>)}
