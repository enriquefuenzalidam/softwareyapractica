    ...
    <li data-aos-once="true" data-aos='fade' className={`relative flex flex-col rounded-sm p-4 bg-white bg-opacity-40 shadow-md shadow-[rgba(0,0,0,0.5)] `} key={softw.id}>
      ...
      <p className={` grow text-md md:text-lg lg:text-xl font-Oswald text-black text-opacity-80 mb-4`} >
        {softw.softCategs.map((categId, index) => {
          ...
          return (<React.Fragment key={categId}><Link className={` no-underline hover:underline`} href={`/softwarecatgoria?catgoriaId=${categoryId}`} key={categoryId}>{categoryName}</Link>{softw.softCategs.length !== 1 && (index < softw.softCategs.length - 1 && (index < softw.softCategs.length - 2 ? `, ` : ` y `))}</React.Fragment>);
        })}.
      </p>
      {softw.softPrec && (
