...
const FondoCabecera = ({ portada }) => {
    ...
    return (
        <section className={` min-h-20 ${portada ? `h-[24vh] sm:h-[38vh] lg:h-[50vh] max-h-96 ` : ` h-[24vh] `}  relative w-full bg-cover bg-center bg-[#D9EFFC] overflow-hidden  `} id={`inicio`}>
            ...
            <div className={`  absolute top-full left-0 w-full h-16 -translate-y-16 bg-gradient-to-t from-[rgba(0,0,0,0.5)] to-transparent `} />
            <div className={` relative mx-auto max-w-6xl flex w-full h-full items-end justify-start `}>
                <h2 className={`  mx-8 -translate-y-[0rem] sm:-translate-y-[0.31rem] md:-translate-y-[0.39rem] lg:-translate-y-[0.47rem] font-LexendDeca text-opacity-100 text-left w-full text-4xl sm:text-5xl md:text-6xl lg:text-7xl `} style={{ textShadow: `0 0 0.3rem rgba(0,0,0,0.5), 0 0 0.6rem rgba(0,0,0,0.2), 0 0 1.5rem rgba(0,0,0,0.7)` }}>
                    {portada ?
